...
namespace EXE.Services
{
    public sealed class PaypalClient
    {
        ...
        public PaypalClient(string clientId, string clientSecret, string mode)
        {
            ...
            Console.WriteLine($"ClientId: {ClientId}, Mode: {Mode}");
        }
        ...
        public async Task<CreateOrderResponse> CreateOrder(decimal amount, string currency, string reference)
        {
            var auth = await Authenticate();
            var value = amount.ToString("F2");
            ...
            if (!httpResponse.IsSuccessStatusCode)
            {
                ...
                Console.WriteLine($"Error creating PayPal order: {jsonResponse}");
